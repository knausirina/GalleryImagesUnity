...
public class SimplePhotoPopup : Popup
{
    ...
    [Inject]
    private void Construct(PhotoService photoService, Config config)
    {
    ...
    private void StartLoading()
    {
        ...
        _lastLoadCts = CancellationTokenSource.CreateLinkedTokenSource(destroyCancellationToken);

        LoadData(_currentIndex, _lastLoadCts.Token).Forget();
    }

    private async UniTaskVoid LoadData(int index, CancellationToken token)
    {
        ...
        try
        {
            ...
            var sprite = await _photoService.GetSprite(url, token);
